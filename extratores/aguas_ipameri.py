...
def extrair_aguas_ipameri(caminho_txt):
    print("⚙️  A iniciar extração: ÁGUAS IPAMERI")
    ...
    # ✂️ A TESOURA IPAMERI
    # =========================================================
    # Assim como o DEMAE, a Ipameri usa o "AUTENTICAÇÂO NO VERSO" no canhoto
    ...
    for indice, pagina_texto in enumerate(faturas_separadas, start=1):
        ...
        # 1. METADADOS IPAMERI
        # =========================================================
        # Buscando no rodapé: FATURA:  11/2023  Nº  119044
        ...
        # NOME:  TRIBUNAL DE JUSTICA DO ESTADO DE GOIAS (Pega tudo até encontrar um espaço muito grande)
        ...
        # Esgoto (Pode não ter nessa fatura, mas deixamos preparado)
        ...
        # Resíduo genérico (ex: Tarifa Básica, IRPJ) — não é SMRSU, é
        # "qualquer outra coisa que não água/esgoto". Ver VALOR_OUTRAS_TAXAS
        # em pipeline.py: nunca é zerada por flag do contas.json.
        outras_taxas_f = round(total_f - agua_f - esgoto_f, 2)

        if num_fatura != "NÃO ACHOU" or conta_dv != "NÃO ACHOU":
            print(f"   🔎 RAIO-X IPAMERI (Bloco {indice}):")
    ...
    if dados_finais:
        ...
        print(f"   ✅ IPAMERI: Extração concluída! Foram capturadas {len(df)} faturas deste arquivo.")
        return df
    else:
        print("   ❌ IPAMERI: O ficheiro não continha faturas válidas.")
        return None